...
public class PlanificacionService : IPlanificacionService
{
    ...
    public async Task<EquipoPlanificacionConfigResult> GetEquipoPlanificacionConfigAsync(string equipoId)
    {
        ...
        return new EquipoPlanificacionConfigResult(
            ...
            configuracion?.NocturnosConsecutivos ?? false);
    }
    ...
    public async Task<Result> SaveEquipoPlanificacionConfigAsync(
        ...
        bool? nocturnosConsecutivos = null)
    {
        if (string.IsNullOrWhiteSpace(equipoId))
        {
        ...
        try
        {
            ...
            var normalizedRequests = normalRequests
                ...
                .Select(r => new
                {
                    ...
                    NumeroPersonasMinimo = NormalizarNumeroPersonasMinimo(r.NumeroPersonas, r.NumeroPersonasMinimo)
                })
            ...
            if (grupo != null && normalizedTurnosOpcionalesVacacion.Count > 0)
            {
                normalizedTurnosOpcionalesVacacion = normalizedTurnosOpcionalesVacacion
                    .DistinctBy(item => $"{item.Dia}|{item.TipoTurnoId}", StringComparer.OrdinalIgnoreCase)
                    .ToList();
            }
            ...
            foreach (var request in normalizedRequests)
            {
                await _db.Planificaciones.AddAsync(new Planificacion
                {
                    ...
                    NumeroPersonasMinimo = request.NumeroPersonasMinimo,
                    IsAuxiliar = false,
                    UsaSoloSecundarios = usaSoloSecundarios,
                    GrupoFuenteSecundariosId = grupoFuenteSecundariosId,
                    UsarPersonaUnicaPorSemana = usarPersonaUnicaPorSemana
        ...
        bool balancearHorasSemanales = true)
    {
        try
        {
            ...
            foreach (var grupoId in gruposIds)
            {
                ...
                gruposEquipo.Add(new GrupoEquipo
                {
                    ...
                    PlanificacionSemanal = planificacionesNormales.Select(p => new PlanificacionTurno
                    {
                        ...
                        CantidadMinima = p.NumeroPersonasMinimo,
            ...
            foreach (var grupo in equipoPlantilla.Grupos)
            {
                ...
                foreach (var planificacion in planificacionOrdenada)
                {
                    ...
                    turnosNormales.Add(new Turno
                    {
                        ...
                        MinimoFlexible = Math.Max(0, planificacion.CantidadMinima),
            ...
            var opcionesSolver = new OpcionesSolverRotacion
            {
                ...
                NivelAgruparTiposTurnoConsecutivos = NivelAgruparTiposTurnoConsecutivos.Bajo,
        ...
        ProblemaRotacion problema)
    {
        ...
        var configurables = problema.Reglas.Configurables;
        if (configurables.GruposEspecialesPersonaUnicaPorSemana.Count > 0 && configurables.GrupoFuentePorGrupoEspecial.Count > 0)
        {
            foreach (var grupoEspecialId in configurables.GruposEspecialesPersonaUnicaPorSemana)
            {
                if (!configurables.GrupoFuentePorGrupoEspecial.TryGetValue(grupoEspecialId, out var grupoFuenteId))
                {
                    continue;
                }
                ...
                var historialGrupo = historialPrevio
                    .Where(item => string.Equals(item.GrupoId, grupoEspecialId, StringComparison.OrdinalIgnoreCase)
                                   && elegibles.Contains(item.PersonaId))
                    .Select(item => new
                    {
                        item.PersonaId,
                        SemanaInicio = ObtenerInicioSemanaLunes(item.FechaTurno)
                    })
                    .GroupBy(item => item.SemanaInicio)
                    .OrderByDescending(grupo => grupo.Key)
                    .ToList();

                if (historialGrupo.Count == 0)
                {
                    continue;
                }

                var semanaMasReciente = historialGrupo[0]
                    .Select(item => item.PersonaId)
                    .ToHashSet(StringComparer.OrdinalIgnoreCase);
                if (semanaMasReciente.Count > 0)
                {
                    estado.EmpleadosGrupoEspecialSemanaAnterior[grupoEspecialId] = semanaMasReciente;
                }

                var usadosCiclo = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                var tamanoCiclo = elegibles.Count;

                foreach (var semana in historialGrupo)
                {
                    var personasSemana = semana
                        .Select(item => item.PersonaId)
                        .Distinct(StringComparer.OrdinalIgnoreCase)
                        .ToArray();

                    var reiniciar = false;
                    foreach (var personaId in personasSemana)
                    {
                        if (!elegibles.Contains(personaId))
                        {
                            continue;
                        }

                        if (usadosCiclo.Contains(personaId))
                        {
                            reiniciar = true;
                            break;
                        }

                        usadosCiclo.Add(personaId);
                        if (usadosCiclo.Count >= tamanoCiclo)
                        {
                            usadosCiclo.Clear();
                            reiniciar = true;
                            break;
                ...
                if (usadosCiclo.Count > 0)
                {
                    estado.EmpleadosGrupoEspecialCicloActual[grupoEspecialId] = usadosCiclo;
                    foreach (var personaId in usadosCiclo)
                    {
                        estado.UsosGrupoEspecialPorEmpleado[(grupoEspecialId, personaId)] = 1;
                    }
                }
            }
        }

        return estado.UltimoFinTurnoPorEmpleado.Count > 0 ||
               estado.EmpleadosGrupoEspecialSemanaAnterior.Count > 0 ||
               estado.EmpleadosGrupoEspecialCicloActual.Count > 0
            ? estado
            : null;
    }

    private static DateOnly ObtenerInicioSemanaLunes(DateOnly fecha)
    {
        var ajuste = fecha.DayOfWeek switch
        {
            DayOfWeek.Monday => 0,
            DayOfWeek.Tuesday => 1,
        ...
        ConfiguracionVisibilidadFeriado? configuracion)
    {
        ...
        var slots = problema.Slots
            .Select(slot =>
            {
                ...
                // Nunca bajar de min_personas configurado, aunque la cobertura feriado sea 0 o no haya match.
                requeridos = Math.Max(slot.MinimoFlexible, requeridos);
                _logger.LogInformation(
    ...
    private static int NormalizarNumeroPersonasMinimo(int numeroPersonas, int numeroPersonasMinimo)
    {
        var cobertura = Math.Max(0, numeroPersonas);
        if (cobertura == 0)
        {
            return 0;
        }

        var minimo = Math.Max(0, numeroPersonasMinimo);
        return Math.Min(cobertura, minimo);
    }
    ...
    private async Task<ReglasRotacion> ConstruirReglasRotacionDesdePlanificacionAsync(string equipoId, bool balancearHorasSemanales = true)
    {
        ...
        return ConstruirReglasRotacion(
            ...
            configEquipo.NocturnosConsecutivos);
    }
        ...
        bool nocturnosConsecutivos = false)
    {
        ...
        return new ReglasRotacion
        {
            ...
            Configurables = new PoliticasConfigurablesEquipo
            {
                ...
                MaximoTurnosNocturnosPorMes = maximoTurnosNocturnosPorMes,
                MaximoTurnosNocturnosPorSemana = maximoTurnosNocturnosPorSemana,
                NocturnosConsecutivos = nocturnosConsecutivos,
        ...
        bool? nocturnosConsecutivos = null)
    {
        if (!maximoSlotsFinSemanaPorMes.HasValue
            && !maximoTurnosNocturnosPorMes.HasValue
            && !maximoTurnosNocturnosPorSemana.HasValue
            && !nocturnosConsecutivos.HasValue)
        {
            return;
        }
        ...
        if (nocturnosConsecutivos.HasValue)
        {
            configuracion.NocturnosConsecutivos = nocturnosConsecutivos.Value;
        }
    }
        ...
        out string detalle)
    {
        ...
        {
            var requeridosFactibilidad = slot.PuedeOmitirsePorVacacion
                ? slot.MinimoFlexible
                : slot.EmpleadosRequeridos;
            ...
            if (elegibles < requeridosFactibilidad)
            {
                conflictos.Add(
                    $"S{slot.IndiceSemana + 1} {slot.Fecha:yyyy-MM-dd}: {DescribirGrupo(slot.GrupoId, gruposPorId)}, {DescribirTipoTurno(slot.TipoTurnoId, tiposTurnoPorId)}, requiere {requeridosFactibilidad} y solo hay {elegibles} elegibles");
            }
        }
