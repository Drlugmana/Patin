...
public sealed class EstadoResolucionSemanal
{
    ...
    /// <summary>
    /// Racha de turnos nocturnos consecutivos de cada empleado que terminó justo antes del inicio del horizonte generado.
    /// Se reconstruye desde el historial al crear el estado inicial y se borra tras actualizar el estado de la primera semana.
    /// Permite aplicar la restricción de máximo de nocturnos consecutivos considerando el historial previo a la generación.
    /// </summary>
    public Dictionary<string, int> RachaNocturnaBordeHistoriaPorEmpleado { get; } = new(StringComparer.OrdinalIgnoreCase);
    ...
    /// <summary>
    /// Empleados ya usados dentro del ciclo vigente de cada grupo especial de persona unica.
    /// Mientras existan elegibles fuera de este conjunto, deben priorizarse antes de repetir.
    /// Al completarse el ciclo (todos usados una vez), el conjunto se reinicia.
    /// </summary>
    public Dictionary<string, HashSet<string>> EmpleadosGrupoEspecialCicloActual { get; } = new(StringComparer.OrdinalIgnoreCase);
